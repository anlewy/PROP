...
from torch.nn import CrossEntropyLoss,MarginRankingLoss, NLLLoss
...
class PROP(BertPreTrainedModel):
    ...
    This module comprises the BERT model followed by the two pre-training heads:
        ...
        - the representative word sets predition head.
    ...
    Inputs:
        ...
        `representative_word_sets_label`: optional representative word sets predition(ROP) loss: torch.LongTensor of shape [batch_size]
            with indices selected in [0, 1]. We actually do not use it.

    Outputs:
        if `masked_lm_labels` and `representative_word_sets_label` are not `None`:
            Outputs the loss which is the sum of the masked language modeling loss and the representative word sets
            predition loss.
        if `masked_lm_labels` or `representative_word_sets_label` is `None`:
            Outputs a tuple comprising
            - the masked language modeling logits of shape [batch_size, sequence_length, vocab_size], and
            - the representative word sets logits of shape [batch_size/2, 2].
    """
    def __init__(self, config):
    ...
    def forward(self, input_ids, token_type_ids=None, input_mask=None, masked_lm_labels=None,
                representative_word_sets_label=None):
        
        batch_size = input_ids.size(0)
        sequence_output, pooled_output = self.bert(input_ids, token_type_ids, input_mask,
        ...
        if masked_lm_labels is not None and representative_word_sets_label is not None:
            ...
            pos_logits = logits[:, 0]
            neg_logits = logits[:, 1]
            # `target = 1` assumed the first input should be ranked
            # higher (have a larger value) than the second input, and vice-versa
            rop_label = torch.ones_like(pos_logits)
            rop_loss = marginloss(pos_logits, neg_logits, rop_label)
            loss = masked_lm_loss + rop_loss
            return loss
        else:
            return prediction_scores
